...
export async function GET(request: NextRequest) {
  ...
  const combinedData = flowData.map((photo: any) => {
    ...
    let exif = {
      ...
      DateTimeOriginal:
        photo.info.overExif?.DateTimeOriginal ||
        fileData.filter((asset: any) => asset.assetId === photo.assetId).map((asset: any) => asset.DateTimeOriginal)[0],
      ...
      ExposureTime:
        photo.info.overExif?.ExposureTime || fileData.filter((asset: any) => asset.assetId === photo.assetId).map((asset: any) => asset.ExposureTime)[0],
      FNumber: photo.info.overExif?.FNumber || fileData.filter((asset: any) => asset.assetId === photo.assetId).map((asset: any) => asset.FNumber)[0],
      ISO:
        photo.info.overExif?.ISOSpeedRatings || fileData.filter((asset: any) => asset.assetId === photo.assetId).map((asset: any) => asset.ISOSpeedRatings)[0],
  ...
  if (!flowData || flowData.length === 0) {
    ...
    return Response.json(
      combinedData
        .sort((a: any, b: any) => (new Date(b.exif.DateTimeOriginal).getTime() - new Date(a.exif.DateTimeOriginal).getTime() > 0 ? 1 : -1))
